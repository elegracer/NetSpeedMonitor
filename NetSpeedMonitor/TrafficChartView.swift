...
final class TrafficChartView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let download = samples.map(\.downloadBytesPerSecond)
        let upload = samples.map(\.uploadBytesPerSecond)
        let maximum = max(download.max() ?? 0, upload.max() ?? 0)
        drawSeries(download, maximum: maximum, color: .systemCyan)
        drawSeries(upload, maximum: maximum, color: .systemPurple)
    }

    private func drawSeries(_ values: [Double], maximum: Double, color: NSColor) {
